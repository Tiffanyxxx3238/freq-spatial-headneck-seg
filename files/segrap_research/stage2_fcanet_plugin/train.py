"""
Stage 2A training: AFS-DSN with FcaNet DCT channel-attention branch.

Usage (from segrap_research/):
    python stage2_fcanet_plugin/train.py \\
...
Three-way comparison (same --cache_dir, same --epochs, same --seed):
  Stage 1 baseline : AFS_DSN_V4   ~414 M  (stage1_focal_freq_loss/train.py --model v4 --lambda_ffl 0.0)
  Stage 2 FcaNet   : AFS_DSN_FcaNet ~24 M  ← this script
  Stage 3 FFT      : AFS_DSN_FFT  ~29.4 M  (stage3_fft_branch/train.py)

Differences from stage3_fft_branch/train.py:
  - Model: AFS_DSN_FcaNet (DCT channel attention, no spectral reconstruction)
  - Param-count comparison: FcaNet vs FFT branch vs V4 baseline
"""
import argparse
import gc
import os
...
def parse_args():
    ...
    p.add_argument('--exp_name',     type=str,   default='s2_fcanet')
...
def main():
    ...
    n_fca = sum(p.numel() for p in model.parameters()) / 1e6

    # Param comparison table (instantiated CPU-only for counting, immediately freed)
    ...
    n_v4  = sum(p.numel() for p in _v4_ref.parameters())  / 1e6
    del _fft_ref, _v4_ref

    print(f"AFS_DSN_FcaNet params : {n_fca:.1f} M  ← Stage 2 (this run)")
    print(f"AFS_DSN_FFT    params : {n_fft:.1f} M  (Stage 3 reference)")
    print(f"AFS_DSN_V4     params : {n_v4:.1f} M  (Stage 1 baseline)")
    print(f"vs FFT reduction      : {n_fft / max(n_fca, 1e-3):.1f}×  "
          f"(FcaNet lighter than FFT)")
    print(f"vs V4  reduction      : {n_v4  / max(n_fca, 1e-3):.1f}×")
