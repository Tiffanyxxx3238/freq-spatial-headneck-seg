"""
Stage 5C training: AFS-DSN with FFT-main Residual Dual-Frequency Fusion.

Final Stage 5 variant (see fft_residual_fusion.py for the full background:
Stage 5A's hard Top-K MoE collapsed unpredictably across seeds; Stage 5B's
free channel-wise alpha barely moved from 0.5). FFT is fixed as the primary
branch; FcaNet contributes only a bounded (beta <= 0.2) residual correction.

Usage (from segrap_research/):
    python stage5_moe_router/train_fft_residual.py \\
        --data_root /workspace/data/SegRap2023/Training_Set_120cases/SegRap2023_Training_Set_120cases \\
        --cache_dir /workspace/data/cache \\
        --exp_name  s5c_fft_residual_seed2 \\
        --seed 2 --epochs 100 --output_dir ./results/stage5c

Decision rule (agreed in advance, evaluated against real test-set metrics
after a full training run — NOT something this script computes itself):
  FcaNet baseline: Dice=0.8424, HD95=0.8372, SDice@1mm=0.9560, SDice@2mm=0.9828
  Run seed3 only if seed2 beats FcaNet on a majority of {Dice, HD95, SDice@1mm}
  AND does not lose meaningfully on HD95 specifically. Otherwise stop here
  and fall back to FcaNet as the main line — no further Stage 5 variants.

Differences from stage5_moe_router/train_fixed.py:
  - Model: AFS_DSN_FFTResidual (FFT primary + FcaNet bounded residual, no
    free symmetric mixing weight)
  - Default --output_dir ./results/stage5c
  - After training/eval: dumps model.freq_branch.get_beta_stats() to
    <output_dir>/results/<exp_name>_beta_stats.json
"""
import argparse
import gc
import json
...
def parse_args():
    ...
    p.add_argument('--exp_name',     type=str,   default='s5c_fft_residual')
    p.add_argument('--output_dir',   type=str,   default='./results/stage5c',
...
def main():
    ...
    n_fftres = sum(p.numel() for p in model.parameters()) / 1e6
    ...
    print(f"AFS_DSN_FFTResidual total params : {n_fftres:.1f} M  (FFT primary + FcaNet bounded residual)")
    print(f"AFS_DSN_FcaNet (Stage2)          : {n_fca:.1f} M  (reference)")
    print(f"AFS_DSN_FFT    (Stage3)          : {n_fft:.1f} M  (reference)")

    print(f"Initial beta stats: {model.freq_branch.get_beta_stats()}")
    ...
    for epoch in epoch_bar:
        ...
        beta_stats = model.freq_branch.get_beta_stats()
        tqdm.write(
            f'   [{epoch+1:03d}/{args.epochs}] '
            f'train={train_loss:.4f}  val={val_loss:.4f}  dice={val_dice:.4f}  '
            f'lr={lr:.1e}  beta_mean={beta_stats["mean_beta"]:.4f}  '
            f'⏱ {time_str}{best_tag}'
        )
        ...
        log_csv(log_path, {
            ...
            'lr':         f'{lr:.2e}',
            'beta_mean':  f'{beta_stats["mean_beta"]:.6f}',
            'beta_std':   f'{beta_stats["std_beta"]:.6f}',
        })
    ...
    # ---- Beta statistics ----
    final_beta_stats = model.freq_branch.get_beta_stats()
    print(f"\n=== Final beta stats (FcaNet residual weight per channel, GLOBAL — not per-OAR/per-case) ===")
    for k, v in final_beta_stats.items():
        print(f"  {k}: {v:.4f}")
    beta_path = os.path.join(results_dir, f'{args.exp_name}_beta_stats.json')
    with open(beta_path, 'w') as f:
        json.dump(final_beta_stats, f, indent=2)
    print(f"Saved beta stats -> {beta_path}")
